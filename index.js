...
function title() {
      console.clear()
	  console.log(chalk.bold.red(figlet.textSync('Renz-Bot', {
		font: 'Standard',
	...
	console.log(chalk.yellow(`\n                        ${chalk.green('[ Powered By Renz ]')}\n\n${chalk.yellow('RenzBot')} : ${chalk.white('WhatsApp Bot Multi Device')}\n${chalk.yellow('Follow Insta Renz')} : ${chalk.white('@milk.hoho')}\n${chalk.yellow('Message Me On WhatsApp')} : ${chalk.white('+62 813-9352-6422')}\n${chalk.yellow('Rest Api')} : ${chalk.white('https://christian-id-api.herokuapp.com/docs')}\n${chalk.yellow('Youtube')} : ${chalk.white('https://youtube.com/channel/UCbetUssizXWLgZdDVEFp8Sg')}\n`))
}
...
const connectToWhatsApp = async () => {
	const conn = makeWASocket({
            ...
            auth: state,
            browser: ["RenzBot-Multi-Device", "Safari", "3.0"]
        })
	title()
        store.bind(conn.ev)
